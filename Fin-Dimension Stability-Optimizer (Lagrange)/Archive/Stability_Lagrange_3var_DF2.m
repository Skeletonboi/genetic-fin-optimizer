...
x0 = [0.2,0.1,0.1,2];
TEST = zeros(n,4);
TEST([1,n+1,(2*n)+1,(3*n)+1]) = x0;
for i = 1:n-1
    TEST([1+i,n+1+i,(2*n)+1+i,(3*n)+1+i]) = TEST([i,n+i,(2*n)+i,(3*n)+i])+lagrange(TEST([i,n+i,(2*n)+i,(3*n)+i]))
end
...
mid =  F_s/cos(atan((m+(b/2)-(a/2))/F_s));
l =  (L_r-a)-L_red;
%int = @(s) 1+ (D_nos/2)/(F_s+(D_nos/2));
c =  int*(4*N*(F_s/D_nos)^2)/(1+sqrt(1+(2*mid/(a+b))^2));
...
g = b-(3*a/4); %Tip chord < 1/2*Root Chord
g_a =  lambda*diff(g,a);
g_b = lambda*diff(g,b);
...
%[a,m,lambda] = solve(y1,y2,y3,y5,[a,m,lambda]) %THIS METHOD TAKES
...
F = [y1;y2;y3;y5];
J = [diff(y1,a),diff(y1,b),diff(y1,m),diff(y1,lambda);
    diff(y2,a),diff(y2,b),diff(y2,m),diff(y2,lambda);
    diff(y3,a),diff(y3,b),diff(y3,m),diff(y3,lambda);
    diff(y5,a),diff(y5,b),diff(y5,m),diff(y5,lambda)];

% We now compute F and J values at x0:
F = eval(subs(F,{a,b,m,lambda},P));
J = eval(subs(J,{a,b,m,lambda},P));
syms k1 k2 k3 k4
% We now perform sudo-Gaussian Elimination to find k1,k2,k3,k4:
ge1 = k1*J(1)+k2*J(2)+k3*J(3)+k4*J(4) + F(1);
ge2 = k1*J(5)+k2*J(6)+k3*J(7)+k4*J(8) + F(2);
ge3 = k1*J(9)+k2*J(10)+k3*J(11)+k4*J(12)+ F(3);
ge4 = k1*J(13)+k2*J(14)+k3*J(15)+k4*J(16)+ F(4);

[k1,k2,k3,k4]= solve(ge1,ge2,ge3,ge4,k1,k2,k3,k4);
X = [k1,k2,k3,k4];
end